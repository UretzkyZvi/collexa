...
class CostCalculationService:
    ...
    def calculate_invocation_cost(
        ...
        if input_tokens > 0:
            costs.append(self.calculate_cost(UsageType.INPUT_TOKENS.value, input_tokens, org_id))
        
        if output_tokens > 0:
            costs.append(self.calculate_cost(UsageType.OUTPUT_TOKENS.value, output_tokens, org_id))
    ...
    def set_custom_pricing(
        ...
        logger.info(f"Set custom pricing for org {org_id}: {usage_type} = {rate_cents_per_unit} cents/unit")
    ...
    def remove_custom_pricing(self, org_id: str, usage_type: Optional[str] = None):
        ...
        logger.info(f"Removed custom pricing for org {org_id}, usage_type: {usage_type}")
    ...
    def _get_rate(self, usage_type: str, quantity: int, org_id: Optional[str] = None) -> int:
