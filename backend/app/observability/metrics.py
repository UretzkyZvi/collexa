...
class MetricsCollector:
    ...
    def _labels_match(
        self, series_labels: Dict[str, str], filter_labels: Dict[str, str]
    ) -> bool:
...
def record_request_duration(
    endpoint: str, method: str, duration_ms: float, org_id: str
):
    """Record request duration metrics."""
    labels = {"endpoint": endpoint, "method": method, "org_id": org_id}
    metrics.record_histogram("request_duration_ms", duration_ms, labels)


def increment_agent_invocations(
    agent_id: str, org_id: str, capability: str, status: str
):
