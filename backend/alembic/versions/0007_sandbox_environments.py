...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_sandboxes_agent_id"), "sandboxes", ["agent_id"], unique=False
    )
    ...
    op.create_index(
        op.f("ix_sandbox_runs_sandbox_id"), "sandbox_runs", ["sandbox_id"], unique=False
    )
    ...
    op.create_table(
        ...
        sa.Column("target_system", sa.String(), nullable=True),
        sa.Column(
            "objectives_json", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column(
            "curriculum_json", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
    ...
    op.create_index(
        op.f("ix_learning_plans_agent_id"), "learning_plans", ["agent_id"], unique=False
    )
    ...
    op.create_index(
        op.f("ix_capability_assessments_agent_id"),
        "capability_assessments",
        ["agent_id"],
        unique=False,
    )
...
def downgrade() -> None:
    # Drop RLS policies
    op.execute(
        "DROP POLICY IF EXISTS capability_assessments_org_isolation ON capability_assessments;"
    )
