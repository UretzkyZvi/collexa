...
class TestEnhancedBillingSystem:
    ...
    async def test_budget_scheduler_initialization(self):
        ...
        for expected_job in expected_jobs:
            assert (
                expected_job in job_ids
            ), f"Job {expected_job} not found in scheduled jobs"
    ...
    async def test_budget_violation_alert_formatting(self):
        ...
        with patch.object(
            alert_service.apobj, "notify", return_value=True
        ) as mock_notify:
    ...
    async def test_budget_warning_alert_formatting(self):
        ...
        with patch.object(
            ...
            success = await alert_service.send_budget_warning_alert(
                org_id=self.org_id, warnings=warnings, severity=AlertSeverity.WARNING
            )
    ...
    def test_celery_task_registration(self):
        ...
        for expected_task in expected_tasks:
            assert (
                expected_task in registered_tasks
            ), f"Task {expected_task} not registered"
    ...
    async def test_webhook_processing_with_retry(self):
        ...
        with patch(
            "app.services.billing.webhook_service.WebhookService"
        ) as mock_webhook_service:
            mock_instance = mock_webhook_service.return_value
            mock_instance.process_webhook.side_effect = [
            ...
            with patch("app.services.billing.async_webhook_service.get_db"):
                ...
                try:
                    result = process_webhook_async(
                        payload_b64, "test_signature", "stripe"
                    )
    ...
    async def test_monthly_report_notification(self):
        ...
        report_data = {
            ...
            "agent_usage": {
                "agent-1": {"agent_name": "Test Agent 1", "total_cost_cents": 3000},
                "agent-2": {"agent_name": "Test Agent 2", "total_cost_cents": 2000},
            },
            "budget_status": {
                "has_violations": True,
                "has_warnings": False,
        ...
        with patch.object(
            ...
            success = await alert_service.send_monthly_report_notification(
                org_id=self.org_id, report_data=report_data
            )
    ...
    async def test_scheduler_job_execution(self):
        ...
        with patch("app.services.scheduling.budget_scheduler_service.get_db"), patch(
            "app.services.budget.budget_enforcement_service.BudgetEnforcementService"
        ) as mock_enforcement:
    ...
    async def test_integration_workflow(self):
        ...
        with patch("app.services.scheduling.budget_scheduler_service.get_db"), patch(
            "app.services.budget.budget_enforcement_service.BudgetEnforcementService"
        ) as mock_enforcement, patch(
            "app.services.notifications.alert_service.AlertService.send_budget_violation_alert"
        ) as mock_alert:
            ...
            with patch(
                "app.services.scheduling.budget_scheduler_service.get_db"
            ) as mock_get_db, patch(
                "app.services.scheduling.budget_scheduler_service.BudgetEnforcementService",
                new=mock_enforcement,
            ):
                mock_session = Mock()
                mock_query = Mock()
                Mock()
