...
async def get_metrics(auth=Depends(require_auth)):
    ...
    duration_stats = metrics.get_histogram_stats(
        "request_duration_ms", {"org_id": org_id}
    )

    # Get agent invocation stats
    invocation_stats = metrics.get_histogram_stats(
        "agent_invocation_duration_ms", {"org_id": org_id}
    )
    invocation_count = metrics.get_counter(
        "agent_invocations_total", {"org_id": org_id}
    )
    ...
    return {
        ...
        "api_calls": {
            ...
            "errors": api_errors,
            "success_rate": (
                (api_calls - api_errors) / api_calls if api_calls > 0 else 1.0
            ),
        },
