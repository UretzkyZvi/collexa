...
def upgrade() -> None:
    ...
    op.execute("CREATE POLICY org_isolation_agents ON agents USING (org_id = current_setting('app.org_id', true))")

    op.execute("ALTER TABLE runs ENABLE ROW LEVEL SECURITY")
    op.execute("CREATE POLICY org_isolation_runs ON runs USING (org_id = current_setting('app.org_id', true))")
