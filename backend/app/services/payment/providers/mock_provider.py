...
class MockProvider(PaymentProvider):
    ...
    async def cancel_subscription(self, subscription_id: str) -> Subscription:
        ...
        if not subscription:
            raise SubscriptionNotFoundError(f"Subscription {subscription_id} not found", "mock")
    ...
    async def create_usage_record(
        ...
        logger.info(f"Created mock usage record: {quantity} units for {subscription_item_id}")
        return True
    ...
    def simulate_webhook_event(self, event_type: str, data: Dict[str, Any]) -> WebhookEvent:
