...
async def get_scheduler_status():
    ...
    try:
        ...
        return {"scheduler": status, "timestamp": "2025-01-19T12:00:00Z"}
...
async def get_notification_channels():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to get notification channels"
        )
...
async def test_notifications():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to test notifications: {str(e)}"
        )
...
async def send_system_alert(request: SystemAlertRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to send system alert: {str(e)}"
        )
...
async def generate_monthly_report(org_id: str, month: str):
    ...
    try:
        from app.services.billing.async_webhook_service import (
            generate_monthly_usage_report_async,
        )
...
async def get_system_health():
    ...
    try:
        # Check scheduler
        scheduler_running = (
            budget_scheduler.scheduler.running if budget_scheduler.scheduler else False
        )
        ...
        # Overall health
        overall_healthy = (
            scheduler_running and len(notification_channels) > 0 and celery_healthy
        )
