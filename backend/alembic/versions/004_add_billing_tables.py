...
revision = "0008_billing_tables"
down_revision = "0007_sandbox_environments"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "current_usage_cents", sa.Integer(), nullable=False, server_default="0"
        ),
    ...
    op.create_index(
        "ix_billing_customers_external_id",
        "billing_customers",
        ["external_customer_id"],
    )
    op.create_index(
        "ix_billing_subscriptions_customer_id", "billing_subscriptions", ["customer_id"]
    )
    op.create_index(
    ...
    op.create_index(
        "ix_billing_events_external_id", "billing_events", ["external_event_id"]
    )
    ...
    op.create_index(
        "ix_usage_records_billing_period", "usage_records", ["billing_period"]
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_billing_subscriptions_external_id", table_name="billing_subscriptions"
    )
    op.drop_index(
        "ix_billing_subscriptions_customer_id", table_name="billing_subscriptions"
    )
