...
class AlertService:
    ...
    async def send_monthly_report_notification(
        self, org_id: str, report_data: Dict[str, Any]
    ) -> bool:
        """
        ...
        try:
            ...
            if budget_status.get("has_warnings"):
                body += (
                    f"⚠️ **Budget Warnings:** {len(budget_status.get('warnings', []))}\n"
                )
            ...
            if success:
                ...
                logger.error(
                    f"Failed to send monthly report notification for org {org_id}"
                )
            ...
            logger.error(
                f"Error sending monthly report notification for org {org_id}: {e}"
            )
            return False
