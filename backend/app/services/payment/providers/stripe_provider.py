...
class StripeProvider(PaymentProvider):
    ...
    def __init__(self):
        if not settings.STRIPE_SECRET_KEY:
            raise PaymentProviderConfigError("STRIPE_SECRET_KEY is required", "stripe")
    ...
    async def create_customer(
        ...
        try:
            ...
            raise PaymentProviderError(
                f"Failed to create customer: {str(e)}", "stripe", e
            )
    ...
    async def get_customer(self, customer_id: str) -> Optional[Customer]:
        ...
        try:
            ...
            raise PaymentProviderError(f"Failed to get customer: {str(e)}", "stripe", e)
    ...
    async def update_customer(
        ...
        try:
            ...
            raise CustomerNotFoundError(
                f"Customer {customer_id} not found", "stripe", e
            )
        except stripe.error.StripeError as e:
            logger.error(f"Failed to update Stripe customer {customer_id}: {e}")
            raise PaymentProviderError(
                f"Failed to update customer: {str(e)}", "stripe", e
            )
    ...
    async def create_checkout_session(
        ...
        try:
            ...
            raise PaymentProviderError(
                f"Failed to create checkout session: {str(e)}", "stripe", e
            )
    ...
    async def get_subscription(self, subscription_id: str) -> Optional[Subscription]:
        ...
        try:
            ...
            raise PaymentProviderError(
                f"Failed to get subscription: {str(e)}", "stripe", e
            )
    ...
    async def cancel_subscription(self, subscription_id: str) -> Subscription:
        ...
        try:
            ...
            raise SubscriptionNotFoundError(
                f"Subscription {subscription_id} not found", "stripe", e
            )
        except stripe.error.StripeError as e:
            logger.error(f"Failed to cancel Stripe subscription {subscription_id}: {e}")
            raise PaymentProviderError(
                f"Failed to cancel subscription: {str(e)}", "stripe", e
            )
    ...
    async def create_usage_record(
        ...
        try:
            usage_record_data = {"quantity": quantity, "action": "increment"}
            if timestamp:
                usage_record_data["timestamp"] = timestamp

            stripe.UsageRecord.create(
                subscription_item=subscription_item_id, **usage_record_data
            )
            return True
        except stripe.error.StripeError as e:
            logger.error(f"Failed to create Stripe usage record: {e}")
            raise PaymentProviderError(
                f"Failed to create usage record: {str(e)}", "stripe", e
            )
    ...
    async def get_invoices(self, customer_id: str, limit: int = 10) -> List[Invoice]:
        ...
        try:
            ...
            raise PaymentProviderError(f"Failed to get invoices: {str(e)}", "stripe", e)
    ...
    def verify_webhook_signature(
        ...
        try:
            ...
            raise WebhookVerificationError("Invalid payload", "stripe", e)
        except stripe.error.SignatureVerificationError as e:
            raise WebhookVerificationError("Invalid signature", "stripe", e)
    ...
    def _convert_stripe_customer(self, stripe_customer) -> Customer:
        ...
        return Customer(
            ...
            metadata=(
                dict(stripe_customer.metadata) if stripe_customer.metadata else None
            ),
        )
    ...
    def _convert_stripe_invoice(self, stripe_invoice) -> Invoice:
        ...
        return Invoice(
            ...
            paid_at=(
                ...
                if stripe_invoice.status_transitions
                else None
            ),
