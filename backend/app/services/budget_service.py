...
class BudgetService:
    ...
    def get_budgets_for_org(self, org_id: str, agent_id: Optional[str] = None) -> List[models.Budget]:
    ...
    def record_usage(
        ...
        logger.info(f"Recorded usage: {usage_type} {quantity} units, ${cost_cents/100:.2f} for org {org_id}")
        return usage_record
    ...
    def _calculate_period_bounds(self, now: datetime, period: BudgetPeriod) -> tuple[datetime, datetime]:
    ...
    def _update_budgets_for_usage(self, org_id: str, agent_id: Optional[str], cost_cents: int, now: datetime):
