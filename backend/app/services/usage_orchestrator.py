...
class UsageOrchestrator:
    ...
    async def record_agent_invocation(
        ...
        usage_costs = self.cost_calculator.calculate_invocation_cost(
            input_tokens=input_tokens, output_tokens=output_tokens, org_id=org_id
        )

        # Check budget before recording usage
        total_cost_cents = self.cost_calculator.calculate_total_cost(usage_costs)
        self.budget_enforcement.check_budget_before_usage(
        ...
        # Update budgets
        self.budget_enforcement.update_budgets_for_usage(
            org_id, total_cost_cents, agent_id
        )
    ...
    async def record_learning_usage(
        ...
        # Check budget
        self.budget_enforcement.check_budget_before_usage(
            org_id, cost.cost_cents, agent_id
        )
    ...
    def check_budget_before_invocation(
        ...
        return self.budget_enforcement.check_budget_before_usage(
            org_id=org_id, estimated_cost_cents=estimated_cost, agent_id=agent_id
        )
    ...
    async def _report_usage_to_provider(self, org_id: str, cost_cents: int):
        ...
        try:
            ...
            usage_item_id = (
                await self.subscription_service.get_subscription_usage_item_id(org_id)
            )
            ...
            if success:
                logger.info(
                    f"Reported usage to payment provider: {cost_cents} cents for org {org_id}"
                )
            else:
                logger.warning(
