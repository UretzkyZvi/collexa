...
def patch_stack_auth(monkeypatch):
    ...
    def fake_verify_team(team_id: str, token: str):
        ...
        return {"id": team_id}
    monkeypatch.setattr("app.security.stack_auth.verify_stack_access_token", fake_verify_token)
    monkeypatch.setattr("app.security.stack_auth.verify_team_membership", fake_verify_team)
...
def test_middleware_blocks_missing_token(client):
    res = client.post("/v1/agents", json={"brief": "x"}, headers={"X-Team-Id": "team_1"})
