...
class CheckoutService:
    ...
    async def create_checkout_session(
        ...
        try:
            ...
            billing_customer = (
                await self.customer_service.get_or_create_customer_for_org(org_id)
            )

            # Prepare metadata
            session_metadata = {"org_id": org_id, **(metadata or {})}
            ...
            logger.info(
                f"Created checkout session for org {org_id}: {checkout_session.id}"
            )
            ...
            logger.error(
                f"Unexpected error creating checkout session for org {org_id}: {e}"
            )
            raise
    ...
    async def create_plan_change_checkout(
        ...
        return await self.create_checkout_session(
            ...
            success_url=success_url,
            cancel_url=cancel_url,
            metadata={"type": "plan_change", "new_plan_id": new_plan_id},
        )
    ...
    async def create_trial_checkout(
        ...
        return await self.create_checkout_session(
            ...
            success_url=success_url,
            cancel_url=cancel_url,
            metadata={"type": "trial", "trial_days": trial_days},
        )
