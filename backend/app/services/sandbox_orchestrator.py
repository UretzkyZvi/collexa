...
class SandboxOrchestrator:
    ...
    async def create_sandbox(
        self, agent_id: str, request: SandboxRequest
    ) -> SandboxInfo:
        ...
        try:
            ...
            for service_type in request.required_services:
                custom_config = request.custom_configs.get(
                    service_type, ServiceConfig(service_type=service_type)
                )
    ...
    async def _create_service(
        self, sandbox_id: str, service_type: str, config: ServiceConfig
    ) -> SandboxService:
        ...
        spec_file = tempfile.NamedTemporaryFile(
            mode="w", suffix=f"_{sandbox_id}_{service_type}.yaml", delete=False
        )
        ...
        if add_services:
            for service_type in add_services:
                if service_type not in sandbox.services:
                    config = update_configs.get(
                        service_type, ServiceConfig(service_type=service_type)
                    )
                    service = await self._create_service(
                        sandbox_id, service_type, config
                    )
