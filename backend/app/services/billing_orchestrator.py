...
class BillingOrchestrator:
    ...
    async def setup_organization_billing(
        ...
        billing_customer = await self.customer_service.create_customer_for_org(
            org_id=org_id, email=email, name=name
        )
    ...
    async def change_subscription_plan(
        self, org_id: str, new_plan_id: str, success_url: str, cancel_url: str
    ) -> CheckoutSession:
        """
        ...
        """
        logger.info(
            f"Creating plan change checkout for org {org_id} to plan {new_plan_id}"
        )

        # Get current subscription
        current_subscription = await self.subscription_service.get_subscription_for_org(
            org_id
        )
        ...
        logger.info(
            f"Created plan change checkout {checkout_session.id} for org {org_id}"
        )
        return checkout_session
    ...
    async def get_organization_billing_status(self, org_id: str) -> Dict[str, Any]:
        ...
        dashboard_url = self.customer_service.get_provider_dashboard_url(
            billing_customer.id
        )
    ...
    async def update_organization_billing_info(
        self, org_id: str, email: Optional[str] = None, name: Optional[str] = None
        ...
        return await self.customer_service.update_customer(
            customer_id=billing_customer.id, email=email, name=name
        )
