...
def process_webhook_async(
    self, payload_b64: str, signature: str, provider: str = "stripe"
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.error(
            f"Webhook processing failed (attempt {self.request.retries + 1}): {exc}"
        )
        ...
        if self.request.retries < self.max_retries:
            ...
            logger.critical(
                f"Webhook processing failed permanently after {self.max_retries} retries: {exc}"
            )
            ...
            try:
                ...
                logger.error(
                    f"Failed webhook stored for manual processing: {failed_webhook}"
                )
...
def process_subscription_event_async(
...
def generate_monthly_usage_report_async(org_id: str, month: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": "success", "org_id": org_id, "month": month, "report": report}

    except Exception as exc:
        logger.error(f"Failed to generate monthly usage report for org {org_id}: {exc}")
        return {"status": "failed", "error": str(exc), "org_id": org_id, "month": month}


@celery_app.task(name="billing.send_budget_alert")
def send_budget_alert_async(
    org_id: str, alert_type: str, alert_data: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if alert_type == "violation":
            ...
            success = alert_service.send_budget_violation_alert(
                org_id=org_id, violations=violations, severity=AlertSeverity.CRITICAL
            )
        elif alert_type == "warning":
            warnings = alert_data.get("warnings", [])
            success = alert_service.send_budget_warning_alert(
                org_id=org_id, warnings=warnings, severity=AlertSeverity.WARNING
            )
...
def queue_webhook_processing(
    payload: bytes, signature: str, provider: str = "stripe"
) -> str:
    """
