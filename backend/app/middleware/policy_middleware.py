...
class PolicyEnforcementMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        if not (request.method == "POST" and self._is_agent_invoke_path(request.url.path)):
        ...
        if not user_id or not org_id:
            return JSONResponse(
                {"detail": "Missing user or organization context"}, 
                status_code=403
            )
        ...
        try:
            ...
            if not allowed:
                ...
                return JSONResponse(
                    {"detail": "Policy denied access to capability"}, 
                    status_code=403
                )
