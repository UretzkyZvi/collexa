...
from app.services.payment.protocol import (
    ...
    WebhookEvent, PaymentProviderError
)
...
class BillingService:
    ...
    async def record_usage(self, org_id: str, quantity: int) -> bool:
        ...
        try:
            ...
            subscription_item_id = (subscription.metadata_json or {}).get("metered_item_id")
    ...
    async def _handle_subscription_event(self, event: WebhookEvent):
        ...
        billing_customer = self.db.query(models.BillingCustomer).filter(
            models.BillingCustomer.external_customer_id == customer_id
        ).first()
        ...
        if event.type == "customer.subscription.created":
            await self._create_or_update_subscription(billing_customer, subscription_data)
        elif event.type == "customer.subscription.updated":
            await self._create_or_update_subscription(billing_customer, subscription_data)
        elif event.type == "customer.subscription.deleted":
            await self._cancel_subscription(billing_customer, subscription_data)
    
    async def _create_or_update_subscription(self, billing_customer: models.BillingCustomer, subscription_data: Dict[str, Any]):
        """Create or update subscription record"""
        subscription_id = subscription_data.get("id")
        
        # Check if subscription already exists
        existing = self.db.query(models.BillingSubscription).filter(
        ...
        if existing:
            ...
            existing.current_period_start = datetime.fromtimestamp(subscription_data.get("current_period_start", 0))
            existing.current_period_end = datetime.fromtimestamp(subscription_data.get("current_period_end", 0))
    ...
    async def _cancel_subscription(self, billing_customer: models.BillingCustomer, subscription_data: Dict[str, Any]):
        """Cancel subscription record"""
        subscription_id = subscription_data.get("id")
        
        subscription = self.db.query(models.BillingSubscription).filter(
            models.BillingSubscription.external_subscription_id == subscription_id
        ).first()
