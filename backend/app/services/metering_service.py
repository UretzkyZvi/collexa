...
class MeteringService:
    ...
    async def record_agent_invocation(
        ...
        self.budget_service.check_budget_before_usage(org_id, total_cost_cents, agent_id)
    ...
    async def record_learning_usage(
        ...
        cost = self._calculate_cost("learning_hour", int(learning_hours * 100))  # Convert to centihours
    ...
    async def generate_usage_report(
        ...
        for budget in budgets:
            budget_status.append({
                ...
                "utilization_percent": (budget.current_usage_cents / budget.limit_cents * 100) if budget.limit_cents > 0 else 0,
    ...
    async def _report_usage_to_provider(self, org_id: str, cost_cents: int):
        ...
        try:
            ...
            if success:
                logger.info(f"Reported usage to payment provider: {usage_quantity} cents for org {org_id}")
            else:
                logger.warning(f"Failed to report usage to payment provider for org {org_id}")
                
        except PaymentProviderError as e:
            logger.error(f"Payment provider error reporting usage for org {org_id}: {e}")
